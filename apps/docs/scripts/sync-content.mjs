...
const OUT_ROOT = path.join(APP_DIR, "src", "content", "docs");
...
const TREES = [
  ...
  { src: ".patterns/tea", dest: "patterns/tea" },
  { src: ".patterns/tea-do", dest: "patterns/tea-do" },
  { src: ".decisions", dest: "decisions" },
];

/** Site-owned pages that live in src/content/docs/ and must survive a clean. */
const SITE_OWNED = new Set(["index.mdx"]);
...
async function main() {
  ...
  for (const tree of TREES) {
    ...
    if (!existsSync(srcRoot) || !(await stat(srcRoot)).isDirectory()) {
      throw new Error(
        `sync-content: source tree "${tree.src}" does not exist. Either it moved, or TREES is stale.`,
      );
    }
    ...
    for (const fileAbs of await walk(srcRoot)) {
      ...
      if (raw.startsWith("---\n")) {
        throw new Error(
          `sync-content: ${toPosix(path.relative(REPO_ROOT, fileAbs))} already has frontmatter. ` +
  ...
  if (dangling.length > 0) {
    throw new Error(
      `sync-content: ${dangling.length} link(s) point at files that do not exist:\n  ` +
        dangling.join("\n  "),
    );
  }

  console.log(
    `sync-content: projected ${written} pages from ${TREES.length} trees` +
