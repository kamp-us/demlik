...
# Present live:  asciinema play demo/saga.cast
# Rebuild:       bash demo/record-saga.sh
set -u
DEMO="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"     # demo
...
( cd "$ROOT" && pnpm dlx tsx@4.21.0 -e "
import { runDemo, narrateDemo } from './src/internal/flow/workflow/demo.ts';
...
echo "done. preview live with:  asciinema play demo/saga.cast"
