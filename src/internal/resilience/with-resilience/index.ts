...
export type ResilienceRunCmd<TC extends Cmd> = CmdOf<
  ReturnType<typeof resilienceRunCmdDef<TC, unknown>>
>;
...
> {
  ...
  const runCmd = resilienceRunCmdDef<C, M>();
  ...
  function renameRunCmds(
    ...
    return runCmds.map((r) => runCmd({ key: r.key, input: r.input }));
  }
