...
describe("withResilience — real runtime: fail, retry-timer, succeed", () => {
  ...
  it("performs the target via base.interpret, opens then recovers the breaker, resets retry", async () => {
    ...
    // follow-up Msg the base handler returned — the wrapper never inspects it,
    // it keeps it here for the verbs and delivers it to the base below).
    expect(afterSucceed.$resilience.calls.do_fetch).toEqual({
      phase: "succeeded",
      result: { type: "loaded", body: "BODY(/x)" },
    });
    // ...and the base's own `loaded` cell ran on it.
    expect(afterSucceed.base.body).toBe("BODY(/x)");
...
// ===========================================================================
// The follow-up is DELIVERED (#80). The base handler resolves `M | void`; once
// `succeed` settles the call, a Msg is folded through the base reducer by the
// same delegate-then-partition path a host-dispatched base Msg takes. The slice
// still records it; `void` delivers nothing; a throw never reaches the base.
// ===========================================================================

describe("withResilience — the base handler's follow-up reaches the base reducer", () => {
  const config = { target: "do_fetch" as const };

  it("a resolved Msg runs the base cell after `$resilience:ok`, and stays in the slice", async () => {
    const wrapped = withResilience(makeBase(), config);
    const ctx: FetchCtx = {
    ...
    await runtime.dispatch({ type: "load", url: "/x", at: 1_000 });

    const state = runtime.getState();
    expect(state.base.body).toBe("BODY(/x)");
    expect(state.$resilience.calls.do_fetch).toEqual({
      phase: "succeeded",
      result: { type: "loaded", body: "BODY(/x)" },
    });
    // The delivery is a fold inside the `$resilience:ok` transition, so the
    // log shows the carrier's settle and the base sees the result from it.
    expect(seen).toEqual(["load", "$resilience:ok"]);
    await runtime.stop();
  });

  it("a throwing handler routes to `$resilience:err`; no base cell runs", async () => {
    const wrapped = withResilience(makeBase(), config);
    const ctx: FetchCtx = {
      fetchUrl: async () => {
    ...
    const state = runtime.getState();
    expect(state.base).toEqual({ url: "/x", body: null });
    expect(state.$resilience.calls.do_fetch?.phase).toBe("failed");
    expect(seen).toEqual(["load", "$resilience:err"]);
    await runtime.stop();
  });
  ...
  // A base whose follow-up chains: `loaded` for a page re-emits the target for
      ...
      update: {
        ...
        loaded: (s, m) => [
          ...
          m.body === "last"
            ? [{ type: "log", line: "done" }]
            : [
                { type: "log", line: `got ${m.body}` },
                { type: "do_fetch", url: "/next" },
              ],
        ],
        note: (s) => [s, []],
      },
  ...
  it("a follow-up that re-emits the target is retagged into `$resilience:run`; its non-target Cmd passes through byte-identical", () => {
    ...
    // The whole Cmd stream: the cold carrier for `/first`, then — from the
    // delivered `loaded` — the raw `log` and the carrier for `/next`. No raw
    // `do_fetch` anywhere.
    expect(cmds).toEqual([
      {
        type: "$resilience:run",
      ...
      { type: "log", line: "got page1" },
      {
        type: "$resilience:run",
        key: "do_fetch",
        input: { type: "do_fetch", url: "/next" },
      },
    ]);
    // The re-emitted target re-entered the gate: the call is `running` again.
  ...
  it("end to end, a chained follow-up runs the target again through the composed interpret", async () => {
    ...
    await runtime.dispatch({ type: "load", url: "/first", at: 1_000 });

    expect(fetched).toEqual(["/first", "/next"]);
    expect(logged).toEqual(["got page1", "done"]);
    expect(runtime.getState().base.body).toBe("last");
    expect(seen).toEqual(["load", "$resilience:ok", "$resilience:ok"]);
    await runtime.stop();
  });

  it("a fresh cache hit delivers the cached follow-up in the same transition, with no run", () => {
    const wrapped = withResilience(makeBase(), {
      target: "do_fetch",
      at: atOf,
      cache: { ttlMs: 60_000 },
    });
    ...
    const { state, cmds } = replay(wrapped, {
      msgs: [
        { type: "load", url: "/x", at: 1_000 },
        {
          type: "$resilience:ok",
          key: "do_fetch",
          result: { type: "loaded", body: "CACHED" },
          at: 1_001,
        },
        // `load` resets `body`; the cache serves the settled `loaded` back
        // without a carrier, and the base folds it again.
        { type: "load", url: "/x", at: 2_000 },
      ] as readonly FetchMsg[],
      ctx,
    });

    expect(state.base.body).toBe("CACHED");
    expect(state.$resilience.calls.do_fetch).toEqual({
      phase: "succeeded",
      result: { type: "loaded", body: "CACHED" },
    });
