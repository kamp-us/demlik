/**
 * VENDORED, VERBATIM — DO NOT EDIT BY HAND.
 *
 *   upstream repo:   github.com/kamp-us/phoenix
 *   upstream path:   packages/fabrika-cli/src/lane/emit.ts
 *   upstream commit: 300faa48bee007db46adaa2141e6182ed3e5148b
 *
 * This is fabrika's OWN epic-machine emitter, running here as the golden oracle
 * for a MULTI-PHASE lane. Both committed templates are single-phase, so the
 * phase-advance half of the fold had no document to be tested against; this
 * emitter produces one, and the test asserts against what IT emitted rather
 * than against a hand-drawn belief about the epic machine's shape.
 *
 * `emit.ts` is being rewritten upstream (phoenix #5800 — epic runs are moving
 * from one-PR-per-child to one-PR-per-run), so NOTHING in this repo may depend
 * on the specific states it emits. What is depended on is the GRAMMAR the
 * emitted document is written in — parallel phase regions, `onDone` pairs,
 * guarded two-arm arrays, history targets, `final` nodes — and that is not
 * what is changing.
 *
 * Kept out of biome (see `biome.jsonc`) so it stays byte-comparable with
 * upstream and re-vendoring is a clean diff. The ONLY edits are the lines
 * marked `// VENDOR EDIT`, which repoint an upstream module at its vendored
 * twin here and inline the one-line constants that lived a directory up.
 */
/**
 ...
 * terminate (#5746).
 */
import {type Ref, readTopology} from "./fabrika-dependencies"; // VENDOR EDIT
import {type DeclaredLine, findCycle} from "./fabrika-topology-doc"; // VENDOR EDIT
/** VENDOR EDIT — upstream `../plan/github.ts`, inlined so the fixture stands alone. */
export interface SubIssueLink {
	readonly number: number;
	readonly state: "open" | "closed";
	readonly stateReason: string | null;
}
/** VENDOR EDIT — upstream `../retry-budget.ts`, inlined so the fixture is one file. */
