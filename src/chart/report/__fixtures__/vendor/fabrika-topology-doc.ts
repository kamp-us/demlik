 ...
 *   upstream path:   packages/fabrika-cli/src/ledger/topology-doc.ts
 *   upstream commit: 300faa48bee007db46adaa2141e6182ed3e5148b
 *
 * `emit.ts`'s cycle check is this module's `findCycle` over the union graph, so
 * vendoring the emitter means vendoring this too. Nothing in `src/` proper
 * imports it — it exists to make the emitter runnable.
 *
...
import {readTopology} from "./fabrika-dependencies"; // VENDOR EDIT
