...
import type { CmdOf, MsgOf, Namespaced, StateOf } from "./graph";

type PG = typeof picker;
type PState = StateOf<PG>;
type PCmd = CmdOf<PG>;
type PIn = Namespaced<MsgOf<PG>, "q">;
...
const bySite = compile<PG, PState, MsgOf<PG>, PCmd, "q">(picker, "q", {
  assign: {},
  cells: pCellsBySite,
});
...
const asFn = compile<PG, PState, MsgOf<PG>, PCmd, "q">(picker, "q", {
  assign: {},
  cells: pCells,
});
eq(
...
const liar = compile<PG, PState, MsgOf<PG>, PCmd, "q">(picker, "q", {
  assign: {},
  cells: {
...
const missing = ((): unknown => {
  try {
    compile<PG, PState, MsgOf<PG>, PCmd, "q">(picker, "q", {
      assign: {},
      cells: {
