...
//
// `src/chart/__probes__/` is 48 files that must NOT compile — the compile-time
// guarantees the chart exists to give, each pinned by a program that names one
// mistake. `tsconfig.json` excludes the directory (48 deliberate errors would
...
// green.
//
// The work is one `tsc` over the whole directory (~1s for all 48), so it is a
