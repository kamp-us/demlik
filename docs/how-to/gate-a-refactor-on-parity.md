...
A trace is plain data — `loaded`, the ordered `msgs`, `finalState` — so JSON is
the whole fixture format. Write it once from a real production run and the gate no
...
record log and nothing public re-hydrates it — reach for it as a diagnostic
artifact, not as the fixture the gate reads back.
