...
// alarm CREATION inside the Sub's runner conflates two concerns:
//
...
//     SYNC: `(sub, ctx, dispatch) => Dispose`. Mixing in a check-then-
//     create await chain forces a `void (async () => …)()` wrapper that
//     hides errors and breaks test ordering.
//
// World B: the Sub is pure listener. Its `deps` carry `alarmName` only,
...
export function fromChromeAlarm<S extends Sub<string, AlarmSubData>, M>(
  ...
  return (sub, _ctx, dispatch) => {
    const listener = (alarm: chrome.alarms.Alarm): void => {
      ...
      if (alarm.name !== sub.deps.alarmName) return;
