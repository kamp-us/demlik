...
- `ToolError<T>` / `ToolFailureOf<T>` / `TaggedFailure<E>` — the failure union a router over `T`
  ...
  the kernel's `malformed_result`, and the router's `unknown_tool` / `malformed_args`.
- `DefineAgentConfig.onToolError(outcome, ctx)` — optional. Fires once per failed call, at the
  interpret boundary, with `outcome` typed against this agent's own tools: a `switch` on `_tag`
  narrows the payload and an unhandled failure mode is a compile error. It is awaited, it is not
  re-fired on resume for an outcome already folded, and a throw is contained and warned like
  `onEvent`'s. `ToolErrorContext` is its second argument — the `callId` and the tool `name` the
  model asked for.

Router-minted failures carry their tag the same way, and `kind` / `reason` are written last, so a
payload field of either name can never shadow the discriminant or the model's channel. Code that
