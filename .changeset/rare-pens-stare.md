...
`mountResilientCall(knob, { slice, attempt, onOk, onErr })` returns
...
`mountResilientCall`, plus the types `MountableKnob`, `MountConfig`,
`MountedCell`, `MountedResilientCall`, `Settle` and `SettleFold`.
